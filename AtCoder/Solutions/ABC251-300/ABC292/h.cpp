...
double moreb(int j) {
	...
	int i = j;
	while (i > 0) {
		vec.push_back(bit[i]); i -= i & -i;
	}
	int k = j;
	ll count = 1;
	while (k > 0) {
		if (k & 1) { v.push_back(count); }
		k >>= 1; count *= 2;
	}
	...
	for (int p = v.size() - 1; p >= 0; --p) {
		sum += vec[p]; num += v[p];
		if (sum >= (ll)num * B) flag = true;
	}
	if (flag) return B;
	else {
		ll x = sum / num;
		ll y = sum - x * num;
		double z = (double)y / (double)num;
		return (double)x + z;
	}
}
...
int main() {
	...
	rep(i, n) {
		(void)scanf("%lld", &a[i + 1]);
		add(i + 1, a[i + 1]);
	}
	rep(i, q) {
		int c; ll x; (void)scanf("%d%lld", &c, &x);
		add(c, x - a[c]);
		cout << moreb(n) << endl;
	}
