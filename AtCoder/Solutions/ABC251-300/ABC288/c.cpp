...
//---------------------------------------------------
const int max_n = 2e5;
vi edge[max_n + 5]; //頂点iから出ている辺、先の頂点番号で管理
int checked[max_n + 5];
int n, m;

int mst(){
	int ans = 0;
	repa(i, n) checked[i] = 0;
	repa(i, n) {
		if (!checked[i]) {
			checked[i] = 1;
			qi Q;
			rep(j, edge[i].size()) {
				if (!checked[edge[i][j]]) Q.push(edge[i][j]);
			}
			while (!Q.empty()) {
				int x = Q.front(); Q.pop();
				if (!checked[x]) { 
					ans++; checked[x] = 1; 
					rep(k, edge[x].size()) {
						if (!checked[edge[x][k]]) Q.push(edge[x][k]);
					}
				}
			}
		}
...
int main() {
	...
	int mst_edge_size = mst();
	cout << m - mst_edge_size << endl;
