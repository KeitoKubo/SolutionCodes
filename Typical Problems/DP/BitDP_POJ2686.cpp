...
int main() {
	...
	for (int i = 0; i <= (1 << n) - 2; i++) {
		rep(v, m) {
			if (dp[i][v] != IMPOS) {
				rep(u, m) { //頂点uへ移動
					if (v != u && e[v][u] > 0) { //vからuへ辺が存在する
						rep(k, n) { //k番目のチケット使用
							if (!((i >> k) & 1)) { //使用されていない時のみ
								dp[i | (1 << k)][u] = min(dp[i | (1 << k)][u], dp[i][v] + e[v][u] / t[n - 1 - k]);
							}
						}
					}
				}
	...
	if (ans == IMPOS) cout << "Impossible" << endl;
	else cout << ans << endl;
