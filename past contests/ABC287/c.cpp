...
//---------------------------------------------------
const int max_n = 2e5;
int node_weight[max_n + 5];

int main() {
	//(void)scanf("%d",& );
	//(void)scanf("%d%d",& ,& );
	int n, m; cin >> n >> m;
	repa(i, n) node_weight[i] = 0;
	DisjointSet ds = DisjointSet(n);
	rep(i, m) {
		int x, y; (void)scanf("%d%d", &x, &y);
		node_weight[x]++; node_weight[y]++;
		ds.unite(x - 1, y - 1);
	}

	int node_weight_one = 0, node_weight_two = 0;
	bool not_exist = false;
	repa(i, n) {
		if (node_weight[i] == 1) node_weight_one++;
		else if (node_weight[i] == 2) node_weight_two++;
		else not_exist = true;
	}
	...
	if (not_exist) {
		...
		if (node_weight_one == 2 && node_weight_two == n - 2) {
			bool flag = true;
			rep(i, n-1) {
				if (!ds.same(i, i + 1)) { flag = false; break; }
			}
